...
public class Wordladder2 {
	...
	// Set<List<String>> beg_set = new HashSet<>();
	...
	// beg_set.add(ini);
	// boolean found = false;
	// while (!beg_set.isEmpty() && !found) {
	// Set<List<String>> temp = new HashSet<>(); // prepare for next set
	// for (List<String> l : beg_set) {
	// char[] str = l.get(l.size() - 1).toCharArray(); // get last word
	// // of each list
	...
	// if (wordlist.contains(s) && !l.contains(s)) {
	// List<String> new_l = new ArrayList<>(l);
	// new_l.add(s);
	// temp.add(new_l);
	// if (s.equals(endWord)) {
	// found = true;
	...
	public List<List<String>> findLadders(String start, String end, List<String> wordList) {
		...
		HashMap<String, ArrayList<String>> nodeNeighbors = new HashMap<String, ArrayList<String>>();// Neighbors
																									...
																									// every
																									// node
		HashMap<String, Integer> distance = new HashMap<String, Integer>();// Distance
																			// of
																			// every
																			// node
																			// from
