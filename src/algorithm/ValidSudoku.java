...
public class ValidSudoku {
	...
	private boolean fillSudoku(char[][] board, List<HashSet<Character>> row_map, List<HashSet<Character>> col_map,
			List<HashSet<Character>> mat_map) {
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[row].length; col++) {
				int mat_index = (row / 3) * 3 + col / 3;
				if (board[row][col] == '.') {
					for (char i : sets) {
						if (!row_map.get(row).contains(i) && !col_map.get(col).contains(i)
								&& !mat_map.get(mat_index).contains(i)) {
							board[row][col] = i;
